...
class MC_Dropout_Model(nn.Module):
    ...
    def __init__(self, input_dim, output_dim,
                 num_units, drop_prob, num_layers):
...
class MC_Dropout():
    ...
    def __init__(self, train_data, parameters, data_mean,
                 data_std, val_data=None, wandb_mode=False):
        self.parameters = parameters
        # Specific to this method
        self.drop_prob = self.set_defaults('drop_prob', 0.1)
        self.num_samples = self.set_defaults('num_samples', 150)
        ...
        self.model_name = self.set_defaults('model_name', "MC Dropout")
        ...
        self.y_std = self.data_std[self.y_cols][0]
        self.y_mean = self.data_mean[self.y_cols][0]
        self.x_mean = self.data_mean[self.x_cols]
        self.x_std = self.data_std[self.x_cols]
        ...
        if self.wandb_mode:
            ...
            try:
                ...
                a = "Weights and Biases API (wandb) isn't installed. "
                "Please install it to use. Turning wandb_mode to False."
    ...
    def train(self, net, optimiser, criterion, data_loader):
        ...
        for X, y in data_loader:
            ...
            loss.backward()
            train_loss += loss * X.size(0)           # multiply by batch size
            optimiser.step()
    ...
    def train_model(self, LLP=True, checkpoint=False, checkpoint_path=None):
        ...
        self.net = MC_Dropout_Model(input_dim=self.input_dim,
        ...
        a = f"Batch size should be no more than {len(self.train_data)}, "
        f"but it is {self.batch_size}."
        assert len(self.train_data) >= self.batch_size, a

        if self.wandb_mode:
            wandb.watch(self.net)  # noqa
            wandb.watch(self.criterion)  # noqa
        ...
        if checkpoint:
            assert checkpoint_path is not None, \
                "Please enter a save path for the training checkpoint."
        ...
        print("Number of trainable model parameters: "
              f"{self.count_parameters(self.net)}, number of "
        ...
        if self.val_data is not None:
            return self.net, train_loss.detach().item(), validation_loss.item()
        else:
            return self.net, train_loss.item()
    ...
    def retrain_model(self, _train_data, _batch_size, _epochs,
        ...
        a = f"Batch size should be no more than {len(self.train_data)}, "
        f"but it is {self.batch_size}."
        assert len(_train_data) >= _batch_size, a
        ...
        print("Number of trainable model parameters: "
              f"{self.count_parameters(self.net)}, "
              f"number of training samples: {len(_train_data)}")
    ...
    def generate_samples(self, X, **kwargs):
        ...
        for i in range(num_samples):
            preds = self.net.cpu().forward(X).data.numpy()
            samples.append(preds)
        # Of shape (#samples, #datapoints, #dependent_vars)
    ...
    def run_sampling(self, X, Y, **kwargs):
        ...
        **kwargs :
        ...
        samples, means, stds = self.generate_samples(
    ...
    def make_prediction(self, x_value, verbose=False, plots=False, **kwargs):
        ...
        **kwargs : TYPE
        ...
        x_value = torch.Tensor(pre.normalise(x_value, self.x_mean, self.x_std))

        # Returns unnormalised data
        samples_pred, means_pred, stds_pred = self.generate_samples(
            x_value, num_samples=num_samples)
        ...
        if plots:
            ...
            plt.grid()
            plt.title("Posterior prediction for a single "
                      "X input\nKDE with rug")
        ...
        if verbose:
            ...
            print(f"Using:\n\tX (normalised) = {x_value.numpy()}\n\tNumber "
                  ...
                  f"{means_pred.item():.3f}\n\tStandard deviation: "
                  f"+/-{stds_pred.item():.3f} sigma ({err_percent:.2f}% "
                  "of prediction)\n\n")
