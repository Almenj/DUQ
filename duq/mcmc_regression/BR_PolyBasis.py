...
import cv2                 # Used for animation - "pip install opencv-python"
# sns.set_style('white')
...
class BR_Gauss_plots:
    ...
    def animate_curves(self,
        ...
        for ind, ends in enumerate(end_frame_arr):
            ...
            if self.scaling:
                num_gaussians = (chain_temp.shape[1] - 1) / 3 # noqa
            else:
        ...
        # images = [img for img in os.listdir(savepath) if img.endswith(".png")] # noqa
...
if __name__ == "__main__":
    ...
    #                            Poly_degree=3,
    #                            N=15,
    #                            true_theta=[-0.9, -0.8, 0.7, -0.7],  # You can specify this, otherwise it will be random # noqa
    #                            true_sd=3))
    ...
    # chain = PR.MCMC_Metropolis_Hastings(iterations=20_000, burnin=6_000)
    ...
    Plots.animate_curves(
        ...
        figheight=4,
        uncertainty=6)
    # Plots.curves(X,Y,ymin=-50,ymax=150,xmin=-6,xmax=6, figwidth=4, figheight=4,uncertainty=2) # noqa
    # Plots.curves(X,Y,ymin=-50,ymax=150,xmin=-6,xmax=6, figwidth=4, figheight=4,uncertainty=6) # noqa
